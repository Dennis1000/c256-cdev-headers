...
// IDE Interface
#define IDE_DATA       (*(volatile unsigned char *)0xAFE830)  // 8-Bit Access here Only
#define IDE_ERROR      (*(volatile unsigned char *)0xAFE831)  // Error Information register (only read when there is an error ) - Probably clears Error Bits
#define IDE_SECT_CNT   (*(volatile unsigned char *)0xAFE832)  // Sector Count Register (also used to pass parameter for timeout for IDLE modus Command)
#define IDE_SECT_SRT   (*(volatile unsigned char *)0xAFE833)  // Start Sector Register (0 = 256), so start @ 1
#define IDE_CLDR_LO    (*(volatile unsigned char *)0xAFE834)  // Low Byte of Cylinder Numnber {7:0}
#define IDE_CLDR_HI    (*(volatile unsigned char *)0xAFE835)  //  Hi Byte of Cylinder Number {9:8} (1023-0).
#define IDE_HEAD       (*(volatile unsigned char *)0xAFE836)  // Head, device select, {3:0} HEad Number, 4 -> 0:Master, 1:Slave, {7:5} = 101 (legacy)//
#define IDE_CMD_STAT   (*(volatile unsigned char *)0xAFE837)  // Command/Status Register - Reading this will clear the Interrupt Registers
#define IDE_DATA_LO    (*(volatile unsigned char *)0xAFE838)  // 16-bit access here
